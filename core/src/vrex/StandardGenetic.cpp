...
vector<Genome> StandardGenetic::select(vector<Genome> &currentGen){
    // roulette wheel
...
vector<Genome> StandardGenetic::fit(vector<Genome> &currentGen){
    int signal=1;
...
vector<Genome> StandardGenetic::sex(Genome father, Genome mother){
    ...
    if (Utils::getRandomBetweenZeroAndOne()<sex_rate){
        ...
        children.push_back(Genome(mother,child_1));
        children.push_back(Genome(mother,child_2));
    }else{
...
vector<Genome> StandardGenetic::mutate(vector<Genome> &individuals){
    vector<Genome> xmen;
