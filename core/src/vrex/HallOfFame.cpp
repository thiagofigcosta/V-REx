    ...
    if (!use_extra_args){
        for (Genome *entry:candidates){
            if (dynamic_cast<NeuralGenome*>(entry)){
                notables.push_back(NeuralGenome(*((NeuralGenome*)entry)));
            }else{
                notables.push_back(Genome(*entry));
            }
        }
...
vector<pair<Genome,string>> HallOfFame::joinGenomeVector(vector<Genome*> candidates, vector<string> extraCandidatesArguments){
    ...
    for (size_t i=0;i<candidates.size();i++) {
        ...
        if (dynamic_cast<NeuralGenome*>(gen)){
            out.push_back(pair<Genome,string>(NeuralGenome(*((NeuralGenome*)gen)),extra));
        }else{
            out.push_back(pair<Genome,string>(Genome(*gen),extra));
        }
    }
    return out;
}